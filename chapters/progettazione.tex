...
\intro{In questo capitolo si esporranno le caratteristiche tecniche della soluzione prodotta, descrivendo l'architettura e le scelte progettuali adottate per la telemetria e per i test}\\
...
\begin{itemize}
      ...
      \item \textbf{Strumentazione automatica.} Per i framework pi\`u diffusi (ASP.NET Core,
...
La scelta di \gls{otel}, come richiesto dal requisito RVD-1, permette flessibilità nella scelta
...
\texttt{OpenTelemetry:Enabled = true} in \emph{appsettings}, rispettando così il requisito RFO-5.
...
costruito, senza conoscere i dettagli della pipeline.
...
Per soddisfare il requisito RFD-9, il sistema arricchisce automaticamente ogni span con
...
Per soddisfare il requisito RFO-10, le metriche di tipo \emph{Histogram} espongono degli
\emph{exemplars}: campioni rappresentativi a cui è associato il \texttt{TraceId} della richiesta che li ha
generati. Questo collegamento abilita la navigazione dal pannello di una metrica aggregata alla trace
specifica che ha prodotto il campione anomalo, rendendo immediata la correlazione tra l'anomalia osservata
e la richiesta che ne è all'origine. Il meccanismo è descritto nel dettaglio nella
...
costruire gli oggetti della pipeline \gls{otel} a partire dalla configurazione \emph{appsettings}.
...
Nel sistema, le trace sono prodotte da tre tipologie di sorgenti: strumentazione
automatica HTTP (richieste in ingresso via AspNetCore, chiamate in uscita via HttpClient);
strumentazione automatica database (query SQL Server via SqlClient, query PostgreSQL via
Npgsql); strumentazione manuale dei moduli di Sia.Core tramite le classi
...
Tutti i segnali telemetrici prodotti dal framework, indipendentemente dal modulo di provenienza, condividono un insieme di attributi descrittivi definiti a livello di risorsa. Tra i più importanti vi sono \texttt{service.name}, che identifica l'installazione applicativa, \texttt{sia.product\_type}, che distingue la linea di prodotto, e \texttt{sia.customer\_name}, che identifica il cliente a cui appartiene l'istanza monitorata.
Questi attributi consentono di filtrare e aggregare metriche, \emph{trace} e \emph{log} provenienti da installazioni differenti mantenendo una vista unificata dell'intero ecosistema applicativo.

\subsubsection{Attributi del modulo Sia.Base}

Il modulo \texttt{Sia.Base}, responsabile delle operazioni \gls{crud} e dell'accesso ai dati, utilizza una tassonomia comune per metriche e \emph{trace}. Gli attributi principali descrivono il tipo di operazione eseguita (\texttt{sia.operation}), il metodo infrastrutturale coinvolto (\texttt{sia.method}), il metodo business che ha originato la chiamata (\texttt{sia.caller\_method}), l'entità interessata (\texttt{sia.entity}) e il repository utilizzato (\texttt{sia.repository}).
Ad esempio, una richiesta di inserimento di un ordine potrebbe essere identificata dagli attributi:

\begin{itemize}
      \item \texttt{sia.operation = insert};
      \item \texttt{sia.entity = OrderEntity};
      \item \texttt{sia.repository = OrdersRepository};
      \item \texttt{sia.caller\_method = CreateOrder}.
\end{itemize}
In caso di errore viene inoltre registrato l'attributo \texttt{exception.type}, che permette di distinguere rapidamente problematiche quali \texttt{SqlException} o \texttt{TimeoutException}.

\subsubsection{Attributi del modulo Sia.Auth}

Nel modulo dedicato all'autenticazione, le metriche utilizzano esclusivamente attributi a bassa cardinalità per evitare la proliferazione di serie temporali. Un esempio è il tag \texttt{reason}, associato ai tentativi di autenticazione falliti.
Le \emph{trace} contengono invece informazioni più dettagliate sul processo di autenticazione. Tra gli attributi principali figurano \texttt{auth.method}, che identifica il meccanismo utilizzato per il login, e \texttt{auth.result}, che descrive l'esito dell'operazione con valori quali \texttt{success}, \texttt{unauthorized}, \texttt{error} o \texttt{two\_factor\_required}. Quando viene richiesta l'autenticazione a due fattori, ulteriori attributi specificano la tipologia adottata, ad esempio \texttt{email} oppure \texttt{totp}.

\subsubsection{Attributi del modulo Sia.Grid}

Nel modulo \texttt{Sia.Grid} il tag centrale è \texttt{componentId}, presente
sia sulle trace sia sulle metriche di tutte le operazioni: identifica quale
griglia specifica ha generato il segnale e permette di confrontare le
prestazioni tra componenti diversi.
A seconda dell'operazione eseguita, possono essere presenti ulteriori attributi specifici. Ad esempio, le operazioni di caricamento pagina includono informazioni sulla presenza di procedure SQL associate, mentre le operazioni di eliminazione massiva riportano il nome della procedura utilizzata. Questo approccio permette di correlare rapidamente eventuali rallentamenti o errori a uno specifico componente dell'interfaccia utente.
...
singoli moduli debbano iniettarli manualmente (RFD-9).\\
...
\begin{description}
      \item[un middleware ASP.NET Core] (\texttt{EnrichActivityWithUserMiddleware}), registrato dopo \texttt{UseAuthentication()},
            legge gli attributi \texttt{userId} e \texttt{userName} da \texttt{HttpContext.User} e li
...
linearmente con il numero di utenti, violando RQO-2. La correlazione \emph{metrica~$\to$~utente}
...
\footcite{Book:GoF:DesignPatterns} per risolvere problemi ricorrenti della strumentazione.
...
Il pattern \emph{Factory} è applicato in più punti della pipeline \gls{otel} per mantenere in un
...
\texttt{BaseApplicationBuilder}; il resto della pipeline consuma un oggetto già pronto.\\
...
\begin{itemize}
      ...
      \item i moduli vedono solo istanze pronte all'uso, senza conoscere la pipeline;
...
sul comportamento integrato dell’applicazione, arrivando allo sviluppo di circa
600 test tra frontend e backend.
...
dei repository e la logica associata ai moduli strumentati con OpenTelemetry.\\
...
I moduli backend che sono stati strumentati con questo tipo di test sono 2
per un totale di 51 test. Anche se in seguito, tramite l'uso di regole assegnate agli
agenti AI developer, sono stati creati nuovi test per ogni modifica effettuata al codice
applicativo.
...
In questo caso il numero di test sviluppati è molto maggiore, ci aggiriamo intorno ai 200,
maggior parte facenti parte di \emph{smoke test} che testano gli endpoint di griglie e CRUD (con una copertura del 100\%)
presenti nel DB.
...
I moduli che risultavano essere più importanti da testare erano 4 (Grid, Calendar, Shared, Kanban)
per un totale di circa 300 test.
...
Per questa caratteristica, i test E2E creati, sono stati semplicemente 29, suddivisi in:
5 per l'autenticazione, 7 per il calendario, 3 per componenti crud, 9 per le griglie e
5 per la visualizzazione di tipo kanban.
