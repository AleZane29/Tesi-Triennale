...
\begin{itemize}
    \item \texttt{sia-telemetry-setup}: configurazione del pipeline OTel, \gls{samplerg},
...
Una \emph{skill} è orientata al modulo \gls{crud}, nella quale viene spiegato al \gls{llm} a cosa serve quel test, vincoli specifici 
...
\begin{description}
  \item[Host:] L'applicazione che ospita l'\gls{llm} e avvia le connessioni \gls{mcp}. Può essere
  ...
  \item[Client:] Il componente interno all'host che gestisce il ciclo di vita della
    ...
    riceve risposte.
  \item[Server \gls{mcp}:] Il servizio che espone dati e funzionalità all'\gls{llm}. Può girare
...
\begin{description}
  \item[Sampling] Consente al server di avviare interazioni ricorsive con l'\gls{llm},
...
più comodo per collegare gli \gls{llm} a qualche strumento locale, bensì a \gls{mcp} come 
...
il chiamante è un \gls{llm} che ragiona sul problema, decide autonomamente quali tool invocare e
