...
\begin{risk}{Integrazione tra tecnologie}
      ...
      \risksolution{utilizzo di un approccio incrementale all'integrazione, partendo da configurazioni di base della pipeline \gls{otel} e aumentando progressivamente la complessità. Consultazione della documentazione, utilizzo di ambienti di sviluppo locali per la validazione end-to-end dei segnali, e confronto con il team aziendale per la risoluzione di eventuali criticità}
      \label{risk:tecnologies-integration}
\end{risk}
\vspace{0.2cm}
\begin{risk}{Impatto della telemetria sulle prestazioni}
      \riskdescription{La strumentazione capillare dei moduli applicativi e la raccolta di trace, metriche e log potrebbero introdurre un overhead non trascurabile in produzione, degradando i tempi di risposta delle \gls{api} o generando un volume eccessivo di dati telemetrici}
...
\subsection{Discrepanze rispetto alla pianificazione iniziale}

Il piano di lavoro prevedeva una fase di confronto comparativo tra più modelli
di intelligenza artificiale (GPT-4o, Claude, Gemini) e l'adozione di
\emph{Spec-Kit} per formalizzare le specifiche affidate all'AI. Nella pratica,
\emph{Claude} è stato scelto fin dalle prime settimane sulla base di
un'esperienza pregressa e di un'indicazione aziendale, senza condurre il
confronto comparativo pianificato; analogamente, \emph{Spec-Kit} non è stato
adottato, sostituito nei fatti dalle \emph{skill} procedurali e dagli agenti
specializzati, che hanno
svolto una funzione analoga nel fissare le convenzioni del \emph{framework}.\\
La motivazione è stata pragmatica: \emph{Claude} si è dimostrato fin dai primi
tentativi in grado di produrre risultati di qualità sufficiente da rendere un
confronto sistematico poco conveniente rispetto al monte ore disponibile. Il
tempo risparmiato è stato reinvestito nell'estensione della telemetria a più
moduli del previsto, nello sviluppo di \emph{skill} e agenti riutilizzabili non
pianificati, in una suite di test più ampia e profonda, e nell'introduzione di
funzionalità di telemetria avanzate (arricchimento utente, \gls{exemplarg}) non
previste dal piano originale.
...
L’attività ha riguardato l’identificazione dei moduli del framework da strumentare, la definizione della pipeline \gls{otel} e la progettazione di una tassonomia coerente di metriche, \emph{trace} e attributi condivisi.
...
\subsubsection*{Settimana 5: Implementazione della pipeline OTel e prima suite di test}
La quinta settimana è stata dedicata all'implementazione del \emph{pipeline} \emph{OpenTelemetry} nel \emph{framework} \emph{.NET 9}, alla prima strumentazione dei moduli core e alla realizzazione di una prima suite di test con il supporto degli agenti AI.
Sono stati configurati il \emph{pipeline} di raccolta telemetrica, gli \emph{exporter} \gls{otlp} e la correlazione tra \emph{log} e \emph{trace} tramite \texttt{TraceId} e \texttt{SpanId}.
...
Sono stati sviluppati strumenti e \emph{skill} procedurali per standardizzare la strumentazione dei moduli applicativi, inclusi componenti \emph{Angular}, \gls{api} e moduli di accesso ai dati.
