...
\begin{itemize}
    ...
    \item pannelli di latenza con percentili \emph{p50}, \emph{p95}, \emph{p99} calcolati via
        \texttt{histogram\_quantile};
    \item un pannello \emph{stat} ``Operazioni ultimo \texttt{\$\{bucket\}}'' basato
        sull'\texttt{increase} del \emph{counter} principale;
    \item pannelli di errori con \emph{break-down} per \texttt{exception.type};
    \item un pannello \emph{logs} su Loki filtrato per \texttt{service.name}.
\end{itemize}
