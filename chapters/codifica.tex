...
\intro{Il presente capitolo illustra l'organizzazione del codice prodotto e i dettagli implementativi della soluzione di telemetria, riprendendo le scelte progettuali descritte nel capitolo~\ref{cap:progettazione} e mostrando come si traducono nella struttura effettiva del framework.}\\
...
\begin{itemize}
      \item un livello \textbf{condiviso} (\texttt{Application.Shared.Telemetry} e
            \texttt{BaseApplicationBuilder}), che configura la pipeline \gls{otel},
            registra \gls{samplerg}, \emph{processor} e \emph{exporter}, e definisce le
...
\texttt{ActivitySource} nella pipeline già costruito, senza conoscere i dettagli del
\gls{samplerg} o degli \emph{exporter}.

\section{Pipeline OpenTelemetry}

La pipeline è configurato in \texttt{BaseApplicationBuilder.ConfigureBuilder} tramite
...
\begin{itemize}
      ...
      \item \texttt{AspNetCore} e
            \texttt{Http} di \texttt{OpenTelemetry.Instrumentation} per le \emph{trace} automatiche delle
      ...
      \item \texttt{OpenTelemetry.Exporter.OpenTelemetryProtocol} per l'esportazione \gls{otlp} verso
            il \emph{Collector}.
\end{itemize}
...
provider di \emph{trace} (\texttt{WithTracing}) con \gls{samplerg} e \emph{processor}
...
Serilog.\\
Nell'appendice~\ref{fig:diag-pipeline} è presentato il diagramma delle classi relativo
alla pipeline OpenTelemetry.

\section{Classi di diagnostica per modulo}

Ciascun modulo strumentato espone una classe statica
\texttt{Sia\{Modulo\}Diagnostics}.
...
\begin{itemize}
      ...
      \item il \texttt{Counter} \texttt{ControllerErrors}
            (\texttt{sia.\{modulo\}.controller.errors}) e il singleton
            \texttt{AsControllerDiagnostics} dell'Adapter verso \texttt{IControllerDiagnostics}, per i moduli con controller.
\end{itemize}
Nel codice applicativo, l'istrumentazione manuale di un'operazione avviene con:
\\
\texttt{Sia\{...\}Diagnostics.ActivitySource.StartActivity(\textquotedbl sia.\{...\}.operazione\textquotedbl);}
\\
...
\begin{itemize}
      ...
      \item \texttt{BaseApiController.RecordMetricError(IControllerDiagnostics,\\ Exception, string operation)},
            metodo che effettua l'\emph{unwrap} di \\ \texttt{SiaLoggerException.OriginalException}
            e applica i \emph{tag} \texttt{exception.type} e \texttt{sia.operation}.
\end{itemize}
Nel modulo (esempio riferito a \texttt{Sia.Devextreme.Crud}) la classe \texttt{Diagnostics}
si presenta, nei suoi elementi essenziali, come segue:~\ref{app:listing-adapter}

\section{Middleware di arricchimento utente}
...
\texttt{app.UseAuthentication()}. La sua responsabilità è leggere gli attributi
\texttt{userId} e \texttt{userName} dal \texttt{HttpContext.User} e pubblicarle nel
\gls{baggageg} di \gls{otel}.\\ Un \texttt{ActivityProcessor} custom
...
come \emph{tag} sull'\texttt{Activity}. Nell'appendice~\ref{app:diag-enduser} è possibile consultare
il diagramma delle classi dell'arricchimento utente.

\section{Configurazione degli exemplars}

L'istruzione \texttt{SetExemplarFilter(ExemplarFilterType.TraceBased)} presente nel provider
\texttt{Meter} della pipeline, istruisce
...
\section{Log Serilog con sink OTLP}

Il \emph{sink} OTLP (\texttt{Serilog.Sinks.OpenTelemetry})
è registrato in sostituzione dei \emph{sink} precedenti,
...
\texttt{SpanId}, soddisfacendo il requisito RFO-4 di correlazione \emph{log}-\emph{trace}.
...
Ogni modulo strumentato espone una dashboard Grafana nella cartella\\
\texttt{Sia.\{Modulo\}/Diagnostics/\{nome\}.dashboard.json}. Il file JSON è versionato nel
repository insieme al codice di strumentazione, in modo da garantire il sincronismo tra le
...
\end{itemize}
Il riferimento canonico è la dashboard \texttt{sia-base.dashboard.json} del modulo
\texttt{Sia.Base}, replicata con le dovute personalizzazioni in ogni modulo successivo.

\begin{figure}[h!]
      \centering
      \makebox[\textwidth][c]{%
            \includegraphics[width=1.23\textwidth]{../images/screenshots/DashboardBase.png}
      }
...
\begin{figure}[h!]
      \centering
      \makebox[\textwidth][c]{%
            \includegraphics[width=1.23\textwidth]{../images/screenshots/DashboardBucket.png}
      }
...
\begin{figure}[h!]
      \centering
      \makebox[\textwidth][c]{%
            \includegraphics[width=1.23\textwidth]{../images/screenshots/DashboardLogsTrace.png}
      }
...
il servizio di \emph{HealthChecks} non implementa \texttt{IControllerDiagnostics}
e non espone il counter \texttt{ControllerErrors}.\\
La seconda è la natura istantanea dei dati raccolti. CPU\%, temperatura, GB di
RAM liberi e spazio su disco non sono grandezze cumulative: l'uso di \texttt{Counter} o
\texttt{Histogram} sarebbe improprio. Per rappresentare questo tipo di dato è stato utilizzato l'\texttt{ObservableGauge},
che a differenza di altri strumenti metrici, rappresenta uno stato istantaneo e non un aggregato.
La sua caratteristica fondamentale
è l'inversione del controllo: non è l'applicazione a inviare attivamente il dato, ma è il sistema
di telemetria a recuperarlo periodicamente tramite una funzione di \emph{callback (polling)} al momento della raccolta.
E poiché le letture hardware avvengono solo quando l'endpoint \texttt{/health} viene
interrogato, le gauge leggono un valore in cache, aggiornato dagli
...
\begin{itemize}
      \item una span padre (\texttt{...cycle}) per ogni iterazione del ciclo di notifica che racchiude tutto il percorso per notificare un
            nuovo dato raccolto dal servizio di \emph{HealthCheck};
      \item una span figlia per ciascun controllo hardware eseguito (\texttt{...check}) che espone quale componente hardware è stato
            verificato e il suo stato attuale;
      \item infine, la span (\texttt{...send}) misura la durata dell'invio del report al \emph{manager}
\end{itemize}
Le trace HTTP generate automaticamente da \texttt{HttpClient}
restano figlie di \texttt{...send}: nella stessa trace è quindi possibile
ispezionare sia la logica applicativa sia il dettaglio della chiamata HTTP verso
il \emph{manager}, senza dover correlare manualmente segnali separati.
